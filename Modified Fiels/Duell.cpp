...
const int duell_nettobreite = 640;  // Window Width
const int duell_nettohoehe  = 400;  // Window Height
int duell_bruttobreite;             // Total Window Width Including Frame
int duell_bruttohoehe;              // Total Window Height Including Frame
struct FieldCommon
	{
	int FIELD[10][9];				// The Duall Board
	int FIGW[10][9];				// White Dice Propertys
	int FIGR[10][9];				// Black Dice Propertys
	int KINGW[10];					// White King Properties
	int KINGR[10];					// Black King Properties
	int IBW, IBR, IPW, IPR, IRW, IRR; // Evaluation Values
	int FIGTW, FIGTR, KNGTW, KNGTR; // Cost Values
	int COSTW, COSTR, COSTO, COSTN; // Total Costs
	};
...
int CO[19];							// Cost Factors
int PFL=0, WTHR1, WTHR2;			// Evaluation Threshold
char INP, INN, ILOOK;
...
char infoText1[35]="FOSS Version\0";
char infoText2[63]="Modified and Translated Version of Duell\0";
char infoText3[36]="Cerial Number\0";
// aus EVENT
...
int PMLabel, CMLabel;
unsigned int MOVENO;
char Space[27]="<Space> -> Continue      \0";
char SpaceNewGame[25]="<Space> -> New Game    \0";
char ESC[27]="<ESC> -> Cancle Command  \0";
char DMW[25]="DUELL-MATT For White   \0";
char DMB[25]="DUELL-MATT For Black \0";
char WCM[30]="Without Moves From Computer \0";
char WPM[25]="Without Player Moves \0";
char PZG[25]="Player Move           \0";
char IllMove[28]="You Cant Move Like that!! \0";
...
FILE *pfPR;
char TEXT[33];
char Prot[33]="Logs Of Game__________________ \0";
char Line[33]="______________________________ \0";
char Zurk[33]="_Undo Move____________________ \0";
char Pchg[33]="_Moves Changed Here___________ \0";
char Osch[33]="_Without Thresholds___________ \0";
...
char Tch[4];
char ShowEnd[30]="End of Move\0";
char ShowTitle[54]="This is How the Computer Thinks\0";
char ShowLine[55]="---------------------------------\0";
bool V0_done=FALSE;

HINSTANCE duell_instance;			// Instance of Duell
HWND duell_window;					// Dualls Main Window
HMENU duell_menu;					// Duall Meunus
HDC duell_dc;						// Context Menus?

const int sound_start		= 0;	// Sound New Game
const int sound_compmove	= 1;	// Sound Computer Finished
const int sound_fold		= 2;	// Sound While Moving
const int sound_persmove	= 3;	// Sound Player Finished
const int sound_win			= 4;	// Sound Game Won
const int sound_duell		= 5;	// Sound A Side is in Check
const int anzahl_sounds		= 6;	// Numeber Sounds?
...
HRESULT display::restore()
	{
		...
		MessageBox( duell_window, "Game Cleared", "Note", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
		present();				
		return S_OK;
	}
...
void GameProtocol::WriteGameProtocol()
	{
		...
		{	
			...
			if (GetSaveFileName(&ofnP))
			{
				...
				if ((INP == 'N') || (IS == -1))
					WriteString(Line);
				// darf hier noch nicht geschlossen werden - "May not be closed here yet"
				// fclose(pfPW);
			}
		}
		else
			MessageBox( duell_window, "Game Log is Allready Open", "Duell-Hinweis", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
	}
...
void GameProtocol::CloseProtocol()
	{
		...
		strcpy(TEXT, "_Steps:_\0");
		WriteString(TEXT);
		if (ILOOK == 'J')
			strcpy(TEXT, "_Ahead_\0");
		else
			strcpy(TEXT, "_Normal_\0");
		WriteString(TEXT);
		strcpy(TEXT, "_Chance:\0");
		WriteString(TEXT);
		if (IRAND == 'J')
			strcpy(TEXT, "___A__ \0");
		else
			strcpy(TEXT, "__Out_ \0");
		WriteString(TEXT);
		if (ILOOK == 'J')
		{
			strcpy(TEXT, "_Swell\0");
			WriteString(TEXT);
...
void GameShowfile::ShowPossibleMoves(int K, int is, int& IC, int& IV, FieldCommon& SFld, char* TXT)
	{
		...
		for (i=1; i<3+1-strlen(DATBUF); i++)
			strcat(Buf, " \0");
		strcat(Buf, DATBUF);
		strcat(Buf, " Possible Moves");
		itoa(IV, DATBUF, 10);
		for (i=1; i<3+1-strlen(DATBUF); i++)
			strcat(Buf, " \0");
		strcat(Buf, DATBUF);
		strcat(Buf, " Variations \0");
		...
		if (TXT[0] == 'B')
			...
			strcpy(Buf,"    \0");
		strcat(Buf, "Max. Rating So Far: \0");
		if (NoCutC)
			strcat(Buf, "     \0");
		else
			strcat(Buf, "aCut \0");
...
void GameShowfile::ShowInfoCosts(FieldCommon& SFld, char* TXT)
	{
		...
		WriteStringShowfile(Buf);
		WriteLnShowfile();
		strcpy(Buf, "        White Pieces:\0");
		strcat(Buf, "          \0");
		strcat(Buf, "        Black Pieces:\0");
		WriteStringShowfile(Buf);
		WriteLnShowfile();
		for (I = 1; I <= 8; I++)
		{
		...
		strcpy(Buf, "        White King: \0");
		strcat(Buf, "          \0");
		strcat(Buf, "        Black King:\0");
		WriteStringShowfile(Buf);
		WriteLnShowfile();
		strcpy(Buf, "\0");
		for (J = 1; J <= 9; J++)
...
class duell
	...
	private:
		int Tab[25][4];			// Cube Rolling Table
		int FOL1[13][13];		// Cube end Value 1
		int FOL2[13][13];		// Cube end Value 2
		int FOL3[13][13];		// Cube end Value 3
		int FOL4[13][13];		// Cube end Value 4
		int prod;
		direct FoldDirect;
		int result;
		DWORD time;				// Time Reference
		int speed;				// Speed	
		void initialisieren();
		void display_spielfeld();
	public:
		int spiel_laeuft;		// Indicator that Game is Running
...
void duell::display()
	{
		...
		TextOutA(duell_dc, 438, 140, GraphicText0, strlen(GraphicText0));
		color = 0x00009800; // Green
		SetTextColor(duell_dc, color);
		TextOutA(duell_dc, 438, 162, GraphicText1, strlen(GraphicText1));
		color = 0x00FF0000; // Blue
		SetTextColor(duell_dc, color);
		TextOutA(duell_dc, 438, 184, GraphicText2, strlen(GraphicText2));
		color = 0x000000FF; // Red
		SetTextColor(duell_dc, color);
		TextOutA(duell_dc, 438, 228, GraphicText3, strlen(GraphicText3));
		TextOutA(duell_dc, 438, 250, GraphicText4, strlen(GraphicText4));
		TextOutA(duell_dc, 438, 272, GraphicText5, strlen(GraphicText5));
		TextOutA(duell_dc, 438, 294, GraphicText6, strlen(GraphicText6));
		SetTextColor(duell_dc, hold_color);
		TextOutA(duell_dc, 438, 316, GraphicText7, strlen(GraphicText7));
		color = 0x000000FF; // Red
		SetTextColor(duell_dc, color);
		TextOutA(duell_dc, 438, 338, GraphicText8, strlen(GraphicText8));
		color = 0x00FF0000; // Blue
		SetTextColor(duell_dc, color);
		TextOutA(duell_dc, 438, 360, GraphicText9, strlen(GraphicText9));
		SetTextColor(duell_dc, hold_color);
...
void duell::Spec()
	{
		if (CM < 0)
		{
			if (IS > 0) strcpy(str,"Its Whites Turn\0");
			else strcpy(str,"Its Blacks Turn\0");
			spec = TRUE;
		}
		else
		{
			if (CT > CM)
			{
				strcpy(str,"Too Many or Few Moves\0");
				spec = TRUE;
			}
		}
	}
...
void duell::retour()
	{
		...
		if (!spec) strcpy(str,"You Cant Move Like This\0");
	}
...
void duell::PersonMove()
	{
		...
		CHFLAG = TRUE; // Responsible for Undoing Moves
		...
		if (PFL >= 2)
		{
			...
			mein_showfile.ShowInfoCosts(W0, "Player Move:");
		}
		if (!Exit)
		{
			if (IS == 1)
...
void duell::ComputerMove()
	{
		...
		W0.COSTO = -W0.COSTN;
		strcpy(GraphicText2, BLANK);
		strcpy(GraphicText2, "Computer is Thinking\0") ;
		IRET = 0;
		Try(IRET);
		// Computing of the cost for a given game-situation with
		...
		{
			// Show the cost infos of the game-situation
			mein_showfile.ShowInfoCosts(W0, "Computer Move:") ;
		}
		if (!Exit)
		{
			// Next move
...
void duell::DuellEnd()
	{
		...
		strcpy(GraphicText6, BLANK);
		strcpy(GraphicText6, "Game Over\0");
...
void duell::ShowCompMove()
	{
		...
		strcpy(GraphicText2, BLANK);
		if (IS == 1)
			strcpy(GraphicText2, "W: Computer Move:  \0");
		else
			strcpy(GraphicText2, "B: Computer Move:  \0");
...
void duell::ErrTest()
	{
		// Error and other messages depending on IERR
		switch(IERR)
		{
		case 0:
			InpOK = TRUE;
		...
		case 3:
			strcpy(GraphicText3, BLANK);
			strcpy(GraphicText4, BLANK);
			strcpy(GraphicText3, "Checkmate? Hit The\0");
			strcpy(GraphicText4, "King in the Path!\0");
			duell_sounds.play(sound_persmove);
			InpOK = TRUE;
			break;
		case 4:
			strcpy(GraphicText6, BLANK);
			strcpy(GraphicText6, "Checkmate ?\0");
			strcpy(GraphicText5, IllMove);
			duell_sounds.play(sound_persmove);
			break;
		default:
...
void duell::Cost(FieldCommon& CFld, int& IS)
	{
		...
		if (IS == 1)
		{
			...
			// ReTry for moves without hitting potential for white with try
			if ((W2FLAG) && (W1.IBW == 0))
			{
				// only hit potential for white at ReTry
				if ((IZW >= IZR) && (CFld.IBW != 0))
					// force beating black figure
					CFld.COSTN = CFld.COSTN - WTHR2;
				// Black's strike potential in the case of a double
				if (W2.IBR != 0)
		...
		else
		{
			...
			// ReTry for moves with no capture potential for Black at Try
			if ((W2FLAG) && (W1.IBR == 0))
			{
				// only hit potential for black with ReTry
				if ((IZR >= IZW) && (CFld.IBR != 0))
					// force beating white figure
					CFld.COSTN = CFld.COSTN - WTHR2;
				// White's strike potential is present in the case of a counter
				if (W2.IBW != 0)
...
// Test Ende
				...
				if (!JumpC)
				{
					...
					if (PFL > 0)
					{
						mein_showfile.ShowPossibleMoves(K, is, ICC, IVC, HbFld, "Counter Moves:");
					}
				}
			}
		}
...
// Test Ende
				...
				if (!JumpR)
				{
					...
					if (PFL > 1)
					{
						mein_showfile.ShowPossibleMoves(K, is, ICR, IVR, HbFld, "Again:");
					}
				}
			}
		}
...
void duell::Try(int& IRET)
	{
		...
		if ((ILOOK == 'J') && (INN != 'S'))
		{
			...
			if (FLDOK)
			{
				Hold(S4, H1);
				W3FLAG = TRUE;
				strcpy(GraphicText9, "Main Variant\0");
			}
		}
		for (K = 1; K <= 9; K++)
		{
			if (!ENDLP_T)
			{
				...
				if (!JUMP)
				{
					...
					if (PFL >= 1)
					{
						mein_showfile.ShowPossibleMoves(K, IS, ICT, IVT, H1, "Searching:");
					}
					ICTOT = ICTOT + ICT;
				}
			}
			...
			for (J = 1; J <= ILIMT; J++)
			{
				if (!ENDLP_T)
				{
					...
					if (!JUMP)
					{
						...
						if (PFL >= 3)
						{
							mein_showfile.ShowInfoCosts(W1, "Searching:");
						}
						if (IRAND == 'J')
							HR = rand()%25 - 12;
						else
...
LRESULT CALLBACK duell_windowhandler( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
			...
			if (M2Flags == 1)
			{
				...
				if (CT != 0)
				{
					if (CT != CM)
					{
						W0.FIELD[xtmp][ytmp] = NewField;
						back = TRUE;
						strcpy(str,"Too Many or Few Moves For the Peice\0");
						spec = TRUE;
						if ((back) && (allowed))
						{
							mein_spiel.retour();
	...
	case WM_RBUTTONDBLCLK:
		if ((!InpOK) && (!K_Flag) && (M1Flags))
		{
			if (CT == CM)
			{
				...
				IQ = 7;
				strcpy(GraphicText0, H_GraphicText);
				strcat(GraphicText0,"Checkmate\0");
				mein_spiel.display();
			}
		}
		return 0;
		...
		if ((mein_spiel.spiel_laeuft) && (Flags == 31))
		{
			...
			else
			{
				...
				if (EndInp)
				{
					...
					ILINE[7] = '\0';
					strcpy(GraphicText1, BLANK);
					strcpy(GraphicText1, "Its Your Turn:  \0");
					strcat(GraphicText1, ILINE);
					mein_spiel.display();
					K_Flag = FALSE;
					mein_spiel.TestMove();
	...
	case WM_COMMAND:
		...
		case ID_BACKWARD:
			...
			{
				if (INN == 'S')
					MessageBox( duell_window, "You cannot undo moves when Spectating a Match", "Duell-Hinweis", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
				else
					MessageBox( duell_window, "You Cant Undo any Moves Right Now", "Duell-Hinweis", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
			}
			else
			{
				duell_sounds.play(sound_persmove);
...
int APIENTRY WinMain( HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR pCmdLine, int nCmdShow)
{
	...
	duell_window = CreateWindowExA( 0, TEXT( "Duell"), TEXT( "Duell"), WS_OVERLAPPEDWINDOW | WS_SYSMENU,
